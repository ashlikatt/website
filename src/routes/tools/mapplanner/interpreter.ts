class TokenError extends Error {
	constructor(s:any, y:any, x:any) {
		super("Compile Error: " + s + "\n At: line " + x + ", column " + y);
	}
}
class CompileError extends Error {
	constructor(s:any, v:any) {
...
class Token {
	...
	constructor(type:any, data:any, x:any, y:any) {
		this.type = type;
		this.data = data;
		this.x = x;
		this.y = y;
...
class SearchNode {
	area:any;
	parent:any;
	constructor(area:any, parent:any = undefined) {
		this.area = area;
		this.parent = parent;
	}
}
function parseMap(tokens:any) {
...
function parseStatement(tokens:any, world:any) {
...
function parseAreaList(tokens:any) {
...
const definedCommands:any = {
	...
	'path': (r:any) => parsePath(currentMap, r),
	'clear': (r:any) => clearConsole(),
	'areas': (r:any) => {
...
function evalCommand(instructions:any) {
	try {
...
function breadth(world:any, start:any, end:any) {
...
function breadthScan(world:any, start:any) {
	const queue = [world.areas[start].node()];
	const explored = new Set();
	explored.add(start);
	while (queue.length > 0) {
...
function areaInfo(world:any, tokens:any) {
...
function parsePath(world:any, tokens:any) {
...
function disjointPath(world:any, tokens:any) {
	...
	if (tokens.length === 1) {
		...
		if (allAreas.length === found.length) {
			...
			printMsg("World is disjoint from " + fromPoint + ". It cannot reach the following areas: " + allAreas.filter((n:any) => !found.includes(n)).join(", "));
		}
		...
		for (const area of allAreas) {
			...
			if (allAreas.length !== found.length) {
				...
				cannotReach = true;
				printMsg(area + " is disjoint. It cannot reach the following areas: " + allAreas.filter((n:any) => !found.includes(n)).join(", "));
			}	
		}
		if (!cannotReach) {
...
class Connection {
	...
	two:any;
	dual:any;
	constructor(one:any, two:any, dual:any) {
		this.one = one;
		this.two = two;
		this.dual = dual;
	}
...
function printCode(world:any, tokens:any) {
	...
	if (type === 'raw') {
		...
		const rawConnections = world.areaList
			.map((n:any) => world.areas[n])
			.map((n:any) => Object.keys(n.connections)
		...
		for (let i = 0; i < rawConnections.length; i++) {
			...
			let index = rawConnections.findIndex((n:any) => n.one === c.two && n.two === c.one);
			if (index > -1) {
				c.dual = true;
				connections.push(c);
...
function areaAcronyms(world:any) {
	const acronyms:any = {};
	const usedAcronyms:any = [];
	for (const name in world.areas) {
		if (name.length <= 3) {
			acronyms[name] = name;
...
function validTriplet(str:any, used:any) {
...
function validPair(str:any, used:any) {
...
function printMsg(m:any) {
	printMsgInline(m + "<br>");
}

function printMsgInline(m:any) {
	consoleBuffer += m;
	textBox.innerHTML = consoleBuffer.replaceAll("\n", "<br>");
	textBox.scroll(0, 9999999);
}

function printErr(m:any) {
...
export function setTextBox(x:any) { 
	mapCode = x; 
	initialCode = mapCode?.value; 
	buildMap(mapCode.value);
}
export function setInputBox(x:any) { 
	inputBox = x; 
    inputBox.addEventListener('keyup', function(e:any) {
        if (e.key === 'Enter') {
...
export function setConsoleBox(x:any) { 
	textBox = x; 
...
export function changedCode(): boolean { return mapCode.value !== initialCode; }