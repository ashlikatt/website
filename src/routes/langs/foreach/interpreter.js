...
export function setConsoleBox(x) { 
	textBox = x; 
}

export function setCodeBox(x) {
	codeBox = x;
	initialCode = codeBox.value;
}

export function setInputBox(x) {
	inputBox = x;
}
...
export function runButton() {
    runForeach(codeBox.value, inputBox.value)
}


export function beforeUnload(e) {
    if (codeBox.value === initialCode) return;
...
export function onKeyDown(e) {
	if (e.ctrlKey && e.key === 's') {
		e.preventDefault();
		download("code.forx", codeBox.value);
	}
};
